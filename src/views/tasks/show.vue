    ...
    <div v-loading="listLoading">
      <el-row class="pb-2">
        <el-col :span="12" class="pl-3">
          ...
          <span>
            {{ licenceDetails.applicationNumber | defaultNull }}
          </span>
        </el-col>
      ...
      <el-row>
        <el-tabs v-model="activeName">
          <el-tab-pane label="Applicant Details" name="first">
            <application-details :application-type="serviceType" :licence-id="licenceId" :licence-details="licenceDetails" @requestPageRefresh="refreshPage" />
          </el-tab-pane>
          ...
          <el-tab-pane v-if="showTaskTab" label="Activities" name="fourth">
            <task-assignment-component
              :licence-id="licenceId"
              :task-id="taskId"
              :available-actors="availableActors"
              :decisions="decisions"
              :licence-forms="licenceForms"
              :licence-task-information="licenceTaskInformation"
              :licence-details="licenceDetailsAndTasks"
              @requestPageRefresh="refreshPage"
            />
          </el-tab-pane>
        </el-tabs>
      </el-row>
...
import taskAssignmentComponent from '@/views/tasks/tasks/taskAssignmentComponent'
import dynamicForm from '@/views/tasks/forms/dynamicForm'
import { statusCodes } from '@/utils/status-codes'
import { getTasksById } from '@/api/tasks'
...
export default {
  ...
  created() {
    ...
    this.licenceId = this.$route.params.applicationId
  ...
  methods: {
    ...
    getAssociatedTasks() {
      ...
      getTasksById(this.taskId).then(response => {
        if (response.data) {
          this.showTaskTab = true
          this.licenceDetailsAndTasks = response.data
          this.licenceTaskInformation = this.licenceDetailsAndTasks?.licence?.track
          this.licenceForms = this.licenceDetailsAndTasks?.activity?.form ? this.licenceDetailsAndTasks?.activity?.form : this.licenceDetailsAndTasks?.form
          this.availableActors = this.licenceDetailsAndTasks?.actors
          this.decisions = this.licenceDetailsAndTasks?.decisions
