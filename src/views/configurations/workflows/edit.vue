  ...
  <div v-loading="listLoading" class="app-container">
    ...
    <div v-if="activeStep === 0">
      ...
      >
        ...
        <el-form-item label="Workflows Type" prop="workflowType" required>
          <el-select
            v-model="wfData.workflowType.id"
            value-key="workflowTypeId"
            style="width: 64vw"
        ...
        <el-form-item>
          <el-button size="mini" plain @click="resetForm('wfRef')">Reset</el-button>
          <el-button size="mini" type="primary" plain style="margin-top: 12px;" @click="next">Next step</el-button>
        </el-form-item>
      </el-form>
    ...
    <div v-if="activeStep === 1">
      ...
      >
        ...
        <el-table-column label="Up & Down" width="120" align="center">
          <template slot-scope="scope">
            <el-tooltip :disabled="scope.$index === 0" content="Up" placement="left">
              <el-button
                plain
                circle
                size="mini"
            ...
            <el-tooltip :disabled="scope.$index + 1 === wfData.steps.length" content="Down" placement="right">
              <el-button
                plain
                circle
                size="mini"
                type="primary"
                :disabled="scope.$index + 1 === wfData.steps.length"
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            ...
            <template v-if="props.row.stepType === 'NORMAL'">
              <p>Current Role: <b>{{
                props.row.currentRole ? props.row.currentRole.name : mapValueTypeById(props.row.currentRoleId, 'roles')
              }}</b></p>
            </template>
            ...
            <template v-else-if="props.row.stepType === 'DOCUMENT_GENERATION'">
              <p>Document: <b>{{
                props.row.attachmentType instanceof Object ? props.row.attachmentType.name : mapValueTypeById(props.row.attachmentType, 'attachmentTypes')
              }}</b></p>
            </template>
            <ul class="ti-list-ol">
              <li v-for="(decision, k) in props.row.decisions" :key="k">Decision: <b>{{ decision.decision }}</b>&middot;
                Template: <b>{{
                  decision.notificationTemplate ? decision.notificationTemplate.displayName : mapValueTypeById(decision.notificationTemplateId, 'notifications')
                }}</b>&middot;
                External Status: <b>{{
                  decision.licenceStatus ? decision.licenceStatus.name : mapValueTypeById(decision.statusId, 'licenceStatuses')
                }}</b></li>
            </ul>
          </template>
        </el-table-column>
        <el-table-column label="Action" align="center">
          <template slot-scope="scope">
            ...
            <!--            <el-button size="mini" type="danger" @click.prevent="deleteStep(scope.row)"> Delete</el-button>-->
          </template>
        </el-table-column>
      </el-table>
      <el-form>
        <el-form-item>
          ...
          <el-button plain size="mini" style="margin-top: 12px;" @click="submitForm">Submit</el-button>
        </el-form-item>
      </el-form>
      ...
      <div>
        ...
        >
          ...
          <template v-if="isNormal">
            ...
            <el-form-item label="Forms" prop="formId">
              <el-select
          ...
          <template v-else-if="isDocumentGeneration">
            <el-form-item label="Possible Decisions" prop="decision">
              <el-checkbox-group v-model="selectedDecision" required>
                <el-checkbox
              ...
              <el-row v-for="(selected, k) in finalDecisions" :key="k" class="pb-2">
                ...
                <el-col :span="8">
                  <el-select
                    ...
                    clearable
                    filterable
                    :value-key="'documentGeneration_' + k"
                    :loading="loading.notification"
                    placeholder="Notification Template"
                  >
                    <el-option
...
export default {
  ...
  created() {
    ...
    const id = this.id = decodeHashId(this.$route.params.Id)
    this.fetchData(id)
  },
  methods: {
    ...
    fetchData(id) {
      ...
      getWorkflowsBy(id).then(response => {
    ...
    submitForm() {
      ...
      data.steps = data.steps.flatMap(step => {
        const tmp = step

        // step due days
        tmp.dueDays = step.dueDays ?? 0
        tmp.formId = tmp.formId ?? (tmp.form?.id ?? 0)
        tmp.currentRoleID = tmp.currentRoleID ?? (tmp.currentRole?.id ?? 0)
        // TODO CHECK tmp.attachmentTypeId
        // tmp.attachmentType = tmp.attachmentType ?? (tmp.attachmentType?.id ?? 0)
        tmp.attachmentType = (tmp.attachmentType instanceof Object)
          ? tmp.attachmentType?.id
          : (tmp.attachmentType ?? 0)
        ...
        tmp.decisions = step.decisions.flatMap(decision => {
          const d = decision

          d.decision = decision.decision
          d.statusId = decision.statusId ?? (decision.licenceStatus?.id ?? 1)
          d.notificationTemplateId = decision.notificationTemplateId ?? (decision.notificationTemplate?.id ?? 0)
      ...
      editWorkflow(this.id, data)
        .then(response => {
          if (response.code === statusCodes.SUCCESS) {
            this.$message({
              message: 'Workflow Edited Successfully.',
              showClose: true,
              type: 'success'
            })
    ...
    editWorkflowStep() {
      this.$refs['wfStepRef'].validate((valid) => {
        if (valid) {
          ...
          // this.wfData.steps[wfStep.index] = wfStep
          this.$set(this.wfData.steps, index, wfStep)
    ...
    handleStepUp(index) {
      // get the index
    // splice the current index
      this.wfData.steps = this.array_move(this.wfData.steps, index, index - 1)
    },
    handleStepDown(index) {
      this.wfData.steps = this.array_move(this.wfData.steps, index, index + 1)
    },
