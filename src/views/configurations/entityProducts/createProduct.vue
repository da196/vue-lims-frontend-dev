<template>
  <div class="app-container">
    <el-steps :active="activeStep" finish-status="success" align-center>
      <el-step title="Create Product" />
      <el-step title="Add Fee Structures" />
    </el-steps>
    <div v-if="activeStep === 1">
      ...
      >
        ...
        <el-form-item label="Products Name" prop="name" required>
          <el-input v-model="licenceProduct.name" />
        </el-form-item>
        <el-form-item label="Products Display Name" prop="displayName" required>
          <el-input v-model="licenceProduct.displayName" />
        </el-form-item>
        <el-form-item label="Products Code" prop="code" required>
          <el-input v-model="licenceProduct.code" />
        </el-form-item>
        <el-form-item label="Applicable State" prop="applicationType" required>
          <el-select v-model="licenceProduct.applicationType" style="min-width: 40vw" placeholder="Select Applicable Type" filterable>
            <el-option v-for="item in applicationType" :key="item.name" :label="item.displayName" :value="item.name" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="Products Description" prop="description" required>
          <el-input v-model="licenceProduct.description" type="textarea" />
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-row :gutter="10">
        <el-col :sm="24" :md="10">
          ...
          >
            ...
            <el-form-item inline-message="Inline code" label="GFS Code" prop="code" required>
              <el-input v-model="standAloneFeeStructure.code" />
            </el-form-item>
            <el-form-item inline-message="Account code" label="Account Code" prop="accountCode" required>
          ...
          <el-table fit highlight-current-row border size="small" :data="licenceProduct.feeStructure">
            <el-table-column type="expand">
              <template slot-scope="props">
                ...
                <p>Applicable State: {{ mapValueTypeById(props.row.applicableState, 'applicableState') }}</p>
              </template>
            </el-table-column>
            ...
            <el-table-column label="Fee Amount" sortable align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.feeAmount }}</span>
              </template>
            </el-table-column>
            <el-table-column label="Fee Currency" sortable align="center">
              <template slot-scope="scope">
                <span>{{ mapValueTypeById(scope.row.feeCurrency, 'currency') }}</span>
              </template>
            </el-table-column>
  ...
  data() {
    return {
      ...
      licenceProduct: {
        ...
        name: '',
        description: '',
        applicationType: 'SHAREHOLDER_CHANGE',
        displayName: '',
        duration: '',
        feeStructure: [],
        workflows: []
      ...
      licenceProduct1Rules: {
        name: [
          {
            required: true,
            message: 'Please input Licence Product name',
            trigger: 'blur'
          },
          { min: 3, message: 'Length should be at least 3', trigger: 'blur' }
        ],
        displayName: [
          {
            required: true,
            message: 'Please input Licence Product Display Name',
            trigger: 'blur'
          },
          { min: 3, message: 'Length should be at least 3', trigger: 'blur' }
        ],
        code: [
          {
            required: true,
            message: 'Please input Licence Product Code',
            trigger: 'blur'
          },
        ...
        description: [
          {
            required: true,
            message: 'Please input Licence Product Description',
            trigger: 'blur'
          },
          { min: 2, message: 'Length should be at least 3', trigger: 'blur' }
        ],
      ...
      currencyTypes: [],
      applicationType: [
        { name: 'SHAREHOLDER_CHANGE', displayName: 'Shareholder Change' },
        { name: 'NAME_CHANGE', displayName: 'Name Change' }
      ],
      applicableStateType: [
  ...
  methods: {
    addFeeStructureToList() {
      this.$refs['feeStructure'].validate((valid) => {
        if (valid) {
          console.log(this.standAloneFeeStructure)
    ...
    fetchLicenceWorkflows() {
      ...
      getWorkflows(0, 1000).then(response => {
    ...
    submitLicenceProduct() {
      ...
      // TODO -- RETURN IT BACK
      createEntityProducts(this.licenceProduct).then(response => {
        this.loadingSave = false
        // this.closeDialog()
        if (response.code === statusCodes.SUCCESS) {
          this.$message({
            message: 'Licence Product created successfully.',
            showClose: true,
            type: 'success'
          })
          // this.resetForm()
