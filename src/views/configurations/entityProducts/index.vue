    ...
    >
      ...
      <el-table-column type="expand" align="center" label="More" width="50">
        <template slot-scope="props">
          <p> <b>Display Name:</b> {{ props.row.displayName }}</p>
...
import {
  ...
  deleteEntityProduct,
  getEntityProductsList
} from '@/api/configurations'
import { statusCodes } from '@/utils/status-codes'
import { encodeHashId } from '@/utils'
import CreateProduct from '@/views/configurations/entityProducts/createProduct'
import EditProduct from '@/views/configurations/entityProducts/editProduct'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getEntityProductsList(this.page.number, this.page.size, this.listQuery.title)
        .then(response => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          createEntityProducts(this.licenceCategory).then(response => {
    ...
    handleView(data) {
      this.$store.dispatch('licences/getEntityProductByEntityProductId', data)
    ...
    handleEdit(data) {
      ...
      // this.editableLicenceProduct.categoryID = data.licenseCategory.id
      this.editableLicenceProduct.feeStructure = data.feeStructures
      this.title = 'Edit Entity Product'
      this.editProductLicence = true
    },
    handleDeactivate(data) {
      this.$confirm('This will permanently delete the entity product. Continue?', 'Warning', {
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancel',
        type: 'warning'
      }).then(() => {
        deleteEntityProduct(data.id).then(response => {
          if (response.code === statusCodes.SUCCESS) {
            this.$message({
