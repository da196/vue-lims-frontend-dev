<template>
  <div class="app-container">
    ...
    <!--      </el-select>-->
    <!--      <el-select-->
    <!--        v-model="listStatus.set"-->
    <!--        :placeholder="'Licence Statuses'"-->
    <!--        clearable-->
    <!--        size="mini"-->
    <!--        class="filter-item"-->
    <!--        style=" margin-left: 10px"-->
    ...
    >
      ...
      <el-table-column align="center" prop="createdAt" label="Date Created">
        <template slot-scope="scope">
...
import { getListOfAllEntityApplications } from '@/api/entities'

export default {
  name: 'EntityApplicationsIndex',
  components: {
    pagination
  },
  filters: {
  ...
  data() {
    return {
      ...
      filterList: [],
      statusList: [{ key: 1, value: 'Paid' }, { key: 2, value: 'Issued' }, { key: 3, value: 'Waiting for Payment' }],
  ...
  methods: {
    ...
    fetchData() {
      ...
      let licence_id = ''
      getListOfAllEntityApplications(this.page.number, this.page.size, 0).then(response => {
        this.listLoading = false
        if (response.data) {
          this.list = response.data.content
          licence_id = response.data.content.id
    ...
    handleView(data) {
      const applicationId = encodeHashId(data.id)
      this.$router.push({ name: 'entity.details',
        params: { applicationId: applicationId, selectedLicenceType: 'ENTITY' }})
    },
    handleBill(data) {
