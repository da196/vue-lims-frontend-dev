      ...
      <el-row class="pb-2">
        ...
        <el-col :span="12" style="text-align: right;" class="pr-3">
          ...
          <span class="success-text">
            {{ licenceDetails.applicationState | defaultNull }}
          </span>
      ...
      <el-row>
        <el-tabs v-model="activeName">
          <el-tab-pane label="Applicant Details" name="first">
            <application-details :application-type="serviceType" :licence-id="entityId" :licence-details="licenceDetails" @requestPageRefresh="refreshPage" @viewGeneratedLicence="viewGeneratedLicence" />
          </el-tab-pane>
...
import { getEntityDetailsById, initiateCancellationOfLicence } from '@/api/licences'
...
export default {
  ...
  created() {
    ...
    this.entityId = this.$route.params.applicationId
    this.entityId = decodeHashId(this.entityId)[0]
    this.fetchData()
  },
  methods: {
    ...
    fetchData() {
      ...
      if (this.serviceType === 'ENTITY') {
        findEntityApplicationById(this.entityId).then(response => {
          this.listLoading = true
          if (response.data) {
        ...
        getEntityDetailsById(this.entityId).then(response => {
          this.listLoading = true
          if (response.data) {
            this.licenceDetails = response.data
            this.listLoading = false
    ...
    submitCancellation() {
      ...
      this.cancellationForm.entityId = this.entityId
